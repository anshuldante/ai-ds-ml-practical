...
* ### Handling of ordinal and continuous variables
  ...
  * F-test:
    ...
    * It's clearly visible in the plot that there is noise in both survivor and non-survivor groups.
  ...
  * Finding the cut-point
    ...
    * The categorical dependent variable is plotted on the y-axis and scale on the x-axis.
...
* ### Handling of missing data
  ...
  * By default, the modeler reads unknown as the 4th value, we can correct that by declaring that the dataset has some missing values for the variable embarked.
  * After declaring that we have some missing data and re-reading the values, the tree now only shows 3 values: C, Q and S.
  * QUEST uses surrogates to fill in the missing values.

* ### Pruning in QUEST

  * QUEST uses the same method as CART to prune the tree.
  ...
  * It splits on multiple variables and even twice for some.
  * Even after pruning, the tree remains the same, which means that QUEST didn't find any branches that could be pruned, which can happen in some cases.
...
* ### Rule sets
  ...
  * Rules for 1, mean survival and rules for zero, mean non-survival.
...
* ### Ensembles
  ...
  * The sample ensemble shows that the Neural Net is not able to predict any cases with missing data, but C5.0 does, this is one way to predict an outcome for all cases and to choose the prediction with the higher confidence value.
...
* ### Understanding Bagging
  ...
  * Although bagging has some pros there are some downsides to it which is why we don't always use it:
...
* ### Random forests
  ...
  * We compare the results of the 2 and as expected, the Random Forest works really well with the train data, but the accuracy goes down significantly when we apply it to test data.
...
* ### Understanding boosting
  ...
  * XGBoost, a flavor or Boosting has become quite famous recently.
  * The improvement in performance of boosted model in case of titanic was modest, but in some scenarios, the boosted models do much better.
